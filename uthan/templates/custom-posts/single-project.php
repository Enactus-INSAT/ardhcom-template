...
<figure>
	<?php if ( class_exists( 'WPuthan_Resizer' ) ) : ?>
		<?php echo wp_kses( $img_obj->WPuthan_resize( wp_get_attachment_url( get_post_thumbnail_id( get_the_ID() ), 'full' ), 1100, 600, true ), $allowed_html ); ?>
	<?php else : ?>
...
<div class="blog-detail-meta">
		...
		<?php if ( $options->get( 'projects_single_categories' ) ) : ?>
			<div class="pro-cat">
				<span class="pro-title"><?php esc_html_e('Posted In:', 'uthan' ); ?></span>
				<span>
				<?php $lists = get_the_terms( get_the_ID(), 'project_cat' ); ?>
					<?php foreach( $lists as $list ) { 
						echo wp_kses(uthan_set( $list,'name' ), true).', ';
					}
					?>
		...
		<div class="opening-hours">
			...
			<ul>
				...
				<li> 
					...
					<?php echo ( uthan_set( $entry, 'opening_day') ) ? '<span>'.uthan_set( $entry, 'opening_day') .':</span>' : ''; ?>

					<?php echo ( uthan_set( $entry, 'opening_time') ) ? '<p>'.uthan_set( $entry, 'opening_time') .'</p>' : ''; ?>
				</li>
				...
				<?php if (  $closed = get_post_meta( get_the_ID(), 'project_closed_day', true ) ) : ?>
					<li>
						...
						<i><?php esc_html_e( 'closed', 'uthan' ); ?></i>
					</li>
				<?php endif; ?>
			</ul>
		</div>
		...
		<?php if ( $options->get( 'projectsingle_post_share' ) && $options->get( 'projectsingle_social_share' ) ) : ?>
			<div class="sharing">
				<span><?php esc_html_e( 'Social Sharing:', 'uthan' ); ?></span>
