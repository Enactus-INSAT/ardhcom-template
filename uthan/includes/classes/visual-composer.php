...
class Visual_Composer {
	...
	function init() {
		...
		$maps = apply_filters( 'uthan_vc_map', $this->maps );
		...
		foreach ( $maps as $value) {
			...
			if ( file_exists( $file ) ) {
				...
				if ( function_exists( 'wputhan_shortcode' ) ) {
					$tag = esc_attr( uthan_set( $data, 'base' ) );

					wputhan_shortcode( $tag, array( $this, 'output' ) );
				}
			}
		}
	}
	...
	function output( $atts, $content = null, $tag ) {
		...
		uthan_template_load( 'shortcodes/' . $tag . '.php', compact( 'atts', 'content', 'tag' ) );
	...
	function get_file( $tag ) {
		...
		$file = uthan_template( 'includes/resource/vc_map/'.$tag . '.php' );
		$file = apply_filters( "uthan_vc_map_file_{$tag}", $file );
	...
	function toggle( $settings, $value ) {
		...
		$values = isset( $settings['value'] ) && is_array( $settings['value'] ) ? $settings['value'] : array( esc_html__( 'Yes', 'uthan' ) => 'true' );
		if ( ! empty( $values ) ) {
