...
class Common {
	...
	function blog( $template = 'blog' ) {
		...
		if ( ( $wp_query->is_posts_page && 'blog' == $template ) || $template == 'page' ) {
			...
			$return = [
				'layout'            => uthan_meta( 'sidebar_sidebar_layout', $page_id, 'right' ),
				'sidebar_type'      => uthan_meta( 'sidebar_source_type', $page_id, 'd' ),
				'sidebar_elementor' => uthan_meta( 'sidebar_elementor_template', $page_id ),
				'sidebar'           => uthan_meta( 'sidebar_page_sidebar', $page_id, 'default-sidebar' ),
				'banner_type'       => uthan_meta( 'banner_source_type', $page_id ),
				'banner_elementor'  => uthan_meta( 'banner_elementor_template', $page_id ),
				'banner'            => uthan_set( uthan_meta( 'banner_page_background', $page_id ), 'url' ),
				'title'             => uthan_meta( 'banner_banner_title', $page_id ),
				'enable_banner'     => uthan_meta( 'banner_page_banner', $page_id ),
			];
		} else {
			...
			$return = [
				...
				'title'         => $options->get( $title ) ? $options->get( $title ) : uthan_the_title( $template ),
				'banner'        => uthan_set( $bg, 'url' ),
