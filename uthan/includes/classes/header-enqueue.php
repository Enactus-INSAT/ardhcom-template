...
class Header_Enqueue {
	...
	public static function scripts() {
		$options = get_theme_mod( 'uthan' . '_options-mods' );
		...
		$scripts = apply_filters( 'UTHAN/includes/classes/header_enqueue/scripts', $scripts );
		/**
		 * Enqueue the scripts
		 *
		 * @var array
		...
		$header_data = array(
			...
			'nonce'   => wp_create_nonce( UTHAN_NONCE ),
		);

		wp_localize_script( 'jquery', 'uthan_data', $header_data );

		if ( uthan_set( $options, 'footer_js' ) ) {

			wp_add_inline_script( 'jquery', uthan_set( $options, 'footer_js' ) );
		}
	...
	public static function styles() {
		...
		$styles = apply_filters( 'UTHAN/includes/classes/header_enqueue/styles', $styles );
		...
		foreach ( $styles as $name => $style ) {
			...
			if ( strstr( $style, 'http' ) || strstr( $style, 'https' ) || strstr( $style, 'fonts.googleapis' ) ) {
				wp_enqueue_style( "uthan-{$name}", $style );
			} else {
				wp_enqueue_style( "uthan-{$name}", get_template_directory_uri() . '/' . $style );
			}
		}
		$options      = uthan_WSH()->option();
		$custom_style = '';
		...
		if ( $custom_font = $options->get('theme_custom_font') ) {
            $header_styles .= uthan_custom_fonts_load( $custom_font );
        }

		wp_add_inline_style( 'uthan-main-style', $header_styles );
	}
	...
	public static function resource_hints( $urls, $relation_type ) {
		if ( wp_style_is( 'uthan-fonts', 'queue' ) && 'preconnect' === $relation_type ) {
			$urls[] = array(
	...
	public static function header_styles() {
		...
		$data = \UTHAN\Includes\Classes\Common::instance()->data( 'blog' )->get();
		...
		$settings = get_theme_mod( 'uthan' . '_options-mods' );

		if ( $custom_font = uthan_set( $settings, 'theme_custom_font' ) ) {

			$styles .= apply_filters('uthan_redux_custom_fonts_load', $custom_font );
