...
class Merlin_Downloader {
	...
	public function download_file( $url, $filename ) {
		...
		$saved_file = uthan_filesystem()->put_contents( $this->download_directory_path . $filename, $content );
		
		if ( ! empty( $saved_file ) ) {
			return $this->download_directory_path . $filename;
		}

		Merlin_Logger::get_instance()->error( __( 'The file was not able to save to disk, while trying to download it', 'uthan' ), array( 'url' => $url, 'filename' => $filename ) );
	...
	private function get_content_from_url( $url ) {
		...
		if ( empty( $url ) ) {
			return new \WP_Error(
				'missing_url',
				__( 'Missing URL for downloading a file!', 'uthan' )
			);
		}
		...
		if ( is_wp_error( $response ) || 200 !== $response['response']['code'] ) {
			...
			return new \WP_Error(
				'download_error',
				sprintf(
					__( 'An error occurred while fetching file from: %1$s%2$s%3$s!%4$sReason: %5$s - %6$s.', 'uthan' ),
					'<strong>',
					$url,
					'</strong>',
					'<br>',
