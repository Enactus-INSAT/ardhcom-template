...
class Importer extends WXRImporter {
	...
	protected function get_reader( $file ) {
		...
		if ( ! class_exists( 'XMLReader' ) ) {
			$this->logger->critical( __( 'The XMLReader class is missing! Please install the XMLReader PHP extension on your server', 'uthan' ) );
		...
		if ( ! $status ) {
			$this->logger->error( __( 'Could not open the XML file for parsing!', 'uthan' ) );
	...
	public function import( $file, $options = array() ) {
		...
		if ( is_wp_error( $result ) ) {
			$this->logger->error( __( 'Content import start error: ', 'uthan' ) . $result->get_error_message() );
	...
	public function new_ajax_request_maybe( $data ) {
		...
		if ( $time > apply_filters( 'pt-importer/time_for_one_ajax_call', 20 ) ) {
			...
			$this->logger->info( __( 'New AJAX call!', 'uthan' ) );
	...
	public function woocommerce_product_attributes_registration( $data ) {
		...
		if ( strstr( $data['taxonomy'], 'pa_' ) ) {
			if ( ! taxonomy_exists( $data['taxonomy'] ) ) {
				...
				if(function_exists('uthan_taxonomy_regster') ) {
					
					// Register the taxonomy now so that the import works!
					uthan_taxonomy_regster(
						$data['taxonomy'],
