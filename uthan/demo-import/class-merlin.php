...
class Merlin {
	...
	function __construct( $config = array(), $strings = array() ) {
		...
		// Get TGMPA.
		if ( class_exists( '\Uthan\Includes\Library\TGMPA' ) ) {
			$this->tgmpa = isset( $GLOBALS['tgmpa'] ) ? $GLOBALS['tgmpa'] : \Uthan\Includes\Library\TGMPA::get_instance();
		}
	...
	public function admin_page() {
		...
		$texts = array(
			'something_went_wrong' => esc_html__( 'Something went wrong. Please refresh the page and try again!', 'uthan' ),
		);
	...
	public function svg( $args = array() ) {
		...
		if ( empty( $args ) ) {
			return esc_html__( 'Please define default parameters in the form of an array.', 'uthan' );
		}

		// Define an icon.
		if ( false === array_key_exists( 'icon', $args ) ) {
			return esc_html__( 'Please define an SVG icon filename.', 'uthan' );
		}
	...
	public function steps() {
		...
		if ( ! empty( $this->import_files ) ) {
			$this->steps['content'] = array(
				'name' => esc_html__( 'Content', 'uthan' ),
				'view' => array( $this, 'content' ),
			);
		}

		$this->steps['ready'] = array(
			'name' => esc_html__( 'Ready', 'uthan' ),
	...
	<?php
		$this->logger->debug( esc_html__( 'The welcome step has been displayed', 'uthan' ) );
	}
	...
	protected function requirements() {
		...
		// Text strings.
		$header    = esc_html__('System Health Check', 'uthan');
	...
	protected function license() {
		...
		<?php
		$this->logger->debug( esc_html__( 'The license activation step has been displayed', 'uthan' ) );
	}
	...
	<?php
		$this->logger->debug( esc_html__( 'The child theme installation step has been displayed', 'uthan' ) );
	}
	...
	protected function plugins() {
		...
		\Uthan\Includes\Library\tgmpa_load_bulk_installer();
					...
					<?php foreach ( $required_plugins as $slug => $plugin ) : ?>
						<li data-slug="<?php echo esc_attr( $slug ); ?>">
							...
							<label for="default_plugins_<?php echo esc_attr( $slug ); ?>">
								...
								<span class="badge">
									<span class="hint--top" aria-label="<?php esc_attr_e( 'Required', 'uthan' ); ?>">
										<?php esc_html_e( 'req', 'uthan' ); ?>
									</span>
								</span>
							</label>
						</li>
	...
	<?php
		$this->logger->debug( esc_html__( 'The plugin installation step has been displayed', 'uthan' ) );
	}
			...
			<?php if ( 1 < count( $this->import_files ) ) : ?>
				...
				<div class="merlin__select-control-wrapper">
					...
					<div class="merlin__select-control-help">
						<span class="hint--top" aria-label="<?php echo esc_attr__( 'Select Demo', 'uthan' ); ?>">
	...
	<?php
		$this->logger->debug( esc_html__( 'The content import step has been displayed', 'uthan' ) );
	}
	...
	<?php
		$this->logger->debug( esc_html__( 'The final step has been displayed', 'uthan' ) );
	}
	...
	public function generate_child() {
		...
		if ( ! file_exists( $path ) ) {
			...
			$this->logger->debug( esc_html__( 'The existing child theme was activated', 'uthan' ) );
		...
		$this->logger->debug( esc_html__( 'The newly generated child theme was activated', 'uthan' ) );
	...
	public function _ajax_activate_license() {
		...
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Yikes! The theme activation failed. Please try again or contact support.', 'uthan' ),
				)
			);
		}
		...
		if ( empty( $_POST['license_key'] ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Please add your license key before attempting to activate one.', 'uthan' ),
				)
			);
		}
		...
		$this->logger->debug( esc_html__( 'The license activation was performed with the following results', 'uthan' ), $result );
	...
	protected function edd_activate_license( $license ) {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				$message = esc_html__( 'An error occurred, please try again.', 'uthan' );
			}
		} else {
				...
				switch ( $license_data->error ) {
					...
					case 'expired':
						$message = sprintf(
							...
							esc_html__( 'Your license key expired on %s.', 'uthan' ),
							date_i18n( get_option( 'date_format' ), strtotime( $license_data->expires, current_time( 'timestamp' ) ) )
						);
						break;

					case 'revoked':
						$message = esc_html__( 'Your license key has been disabled.', 'uthan' );
						break;

					case 'missing':
						$message = esc_html__( 'This appears to be an invalid license key. Please try again or contact support.', 'uthan' );
						break;

					case 'invalid':
					case 'site_inactive':
						$message = esc_html__( 'Your license is not active for this URL.', 'uthan' );
						break;

					case 'item_name_mismatch':
						/* translators: EDD Item Name */
						$message = sprintf( esc_html__( 'This appears to be an invalid license key for %s.', 'uthan' ), $this->edd_item_name );
						break;

					case 'no_activations_left':
						$message = esc_html__( 'Your license key has reached its activation limit.', 'uthan' );
						break;
	...
	public function generate_child_functions_php( $slug ) {
		...
		$this->logger->debug( esc_html__( 'The child theme functions.php content was generated', 'uthan' ) );
	...
	public function generate_child_style_css( $slug, $parent, $author, $version ) {
		...
		$this->logger->debug( esc_html__( 'The child theme style.css content was generated', 'uthan' ) );
	...
	public function generate_child_screenshot( $path ) {
		...
		if ( ! empty( $screenshot ) && file_exists( $screenshot ) ) {
			...
			$this->logger->debug( esc_html__( 'The child theme screenshot was copied to the child theme, with the following result', 'uthan' ), array( 'copied' => $copied ) );
		} else {
			$this->logger->debug( esc_html__( 'The child theme screenshot was not generated, because of these results', 'uthan' ), array( 'screenshot' => $screenshot ) );
		}
	}
	...
	function _ajax_plugins() {
		...
		foreach ( $plugins['activate'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Activating', 'uthan' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['update'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Updating', 'uthan' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['install'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Installing', 'uthan' ),
				);
				break;
			}
		}
		...
		if ( $json ) {
			$this->logger->debug(
				esc_html__( 'A plugin with the following data will be processed', 'uthan' ),
				array(
			...
			$json['message'] = esc_html__( 'Installing', 'uthan' );
			wp_send_json( $json );
		} else {
			$this->logger->debug(
				esc_html__( 'A plugin with the following data was processed', 'uthan' ),
				array(
					'plugin_slug' => $_POST['slug'],
				)
			);
			...
			wp_send_json(
				array(
					'done'    => 1,
					'message' => esc_html__( 'Success', 'uthan' ),
				)
			);
		}
	...
	function _ajax_content() {
		...
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
			$this->logger->error( esc_html__( 'The content importer AJAX call failed to start, because of incorrect data', 'uthan' ) );

			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Invalid content!', 'uthan' ),
				)
			);
		}
		...
		if ( isset( $_POST['proceed'] ) ) {
			if ( is_callable( $this_content['install_callback'] ) ) {
				$this->logger->info(
					esc_html__( 'The content import AJAX call will be executed with this import data', 'uthan' ),
					array(
		...
		if ( $json ) {
			...
			$this->logger->error(
				esc_html__( 'The content import AJAX call failed with this passed data', 'uthan' ),
				array(
			...
			wp_send_json(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Error', 'uthan' ),
					'logs'    => '',
					'errors'  => '',
				)
			);
	...
	public function _ajax_get_total_content_import_items() {
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) && empty( $_POST['selected_index'] ) ) {
			$this->logger->error( esc_html__( 'The content importer AJAX call for retrieving total content import items failed to start, because of incorrect data.', 'uthan' ) );

			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Invalid data!', 'uthan' ),
				)
			);
		}
	...
	protected function get_import_data( $selected_import_index = 0 ) {
		...
		if ( ! empty( $import_files['content'] ) ) {
			$content['content'] = array(
				'title'            => esc_html__( 'Content', 'uthan' ),
				'description'      => esc_html__( 'Demo content data.', 'uthan' ),
				'pending'          => esc_html__( 'Pending', 'uthan' ),
				'installing'       => esc_html__( 'Installing', 'uthan' ),
				'success'          => esc_html__( 'Success', 'uthan' ),
		...
		if ( ! empty( $import_files['widgets'] ) ) {
			$content['widgets'] = array(
				'title'            => esc_html__( 'Widgets', 'uthan' ),
				'description'      => esc_html__( 'Sample widgets data.', 'uthan' ),
				'pending'          => esc_html__( 'Pending', 'uthan' ),
				'installing'       => esc_html__( 'Installing', 'uthan' ),
				'success'          => esc_html__( 'Success', 'uthan' ),
		...
		if ( ! empty( $import_files['sliders'] ) ) {
			$content['sliders'] = array(
				'title'            => esc_html__( 'Revolution Slider', 'uthan' ),
				'description'      => esc_html__( 'Sample Revolution sliders data.', 'uthan' ),
				'pending'          => esc_html__( 'Pending', 'uthan' ),
				'installing'       => esc_html__( 'Installing', 'uthan' ),
				'success'          => esc_html__( 'Success', 'uthan' ),
		...
		if ( ! empty( $import_files['options'] ) ) {
			$content['options'] = array(
				'title'            => esc_html__( 'Options', 'uthan' ),
				'description'      => esc_html__( 'Sample theme options data.', 'uthan' ),
				'pending'          => esc_html__( 'Pending', 'uthan' ),
				'installing'       => esc_html__( 'Installing', 'uthan' ),
				'success'          => esc_html__( 'Success', 'uthan' ),
		...
		if ( ! empty( $import_files['redux'] ) ) {
			$content['redux'] = array(
				'title'            => esc_html__( 'Redux Options', 'uthan' ),
				'description'      => esc_html__( 'Redux framework options.', 'uthan' ),
				'pending'          => esc_html__( 'Pending', 'uthan' ),
				'installing'       => esc_html__( 'Installing', 'uthan' ),
				'success'          => esc_html__( 'Success', 'uthan' ),
		...
		if ( false !== has_action( 'merlin_after_all_import' ) ) {
			$content['after_import'] = array(
				'title'            => esc_html__( 'After import setup', 'uthan' ),
				'description'      => esc_html__( 'After import setup.', 'uthan' ),
				'pending'          => esc_html__( 'Pending', 'uthan' ),
				'installing'       => esc_html__( 'Installing', 'uthan' ),
				'success'          => esc_html__( 'Success', 'uthan' ),
	...
	public function import_revolution_sliders( $file ) {
		...
		$this->logger->info( esc_html__( 'The revolution slider import was executed', 'uthan' ) );
	...
	public function pt_importer_new_ajax_request_response_data( $data ) {
		...
		$data['message']  = esc_html__( 'Installing', 'uthan' );
		$data['proceed']  = 'true';
	...
	public function after_content_import_setup() {
		...
		if ( $homepage ) {
			...
			$this->logger->debug( esc_html__( 'The home page was set', 'uthan' ), array( 'homepage_id' => $homepage ) );
		}
		...
		if ( $blogpage ) {
			...
			$this->logger->debug( esc_html__( 'The blog page was set', 'uthan' ), array( 'blog_page_id' => $blogpage ) );
		}
	}
	...
	public function before_content_import_setup() {
		...
		if ( ! empty( $hello_world ) ) {
			...
			$this->logger->debug( esc_html__( 'The Hello world post status was set to draft', 'uthan' ) );
		}
	}
	...
	public function validate_import_file_info( $import_files ) {
		...
		foreach ( $import_files as $import_file ) {
			if ( ! empty( $import_file['import_file_name'] ) ) {
				...
				$this->logger->warning( esc_html__( 'This predefined demo import does not have the name parameter: import_file_name', 'uthan' ), $import_file );
			}
		}
